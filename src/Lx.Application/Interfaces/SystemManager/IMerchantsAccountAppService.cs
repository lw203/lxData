...
namespace Lx.Application.Interfaces.SystemManager
{
    public interface IMerchantsAccountAppService : IDisposable
    {
        ...
        Task<IEnumerable<MerchantsAccountViewModel>> GetByPage<Tkey>(int pageSize, int pageIndex, string nickName, string phone, string email, DateTime startDate, DateTime endDate);
        Task<int> GetCount(string nickName, string phone, string email, DateTime startDate, DateTime endDate);
